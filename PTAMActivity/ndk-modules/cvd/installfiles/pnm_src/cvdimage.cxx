...
void create_normalized_hist(const Image<byte>& im, std::tr1::array<int,256>& h)
{
...
Huff* create_tree(const std::tr1::array<int,256>& h, vector<Huff*>& symbols)
{
	set<Huff*,Huff> table;
...
struct SortIndex
{
	const std::tr1::array<int, 256>& d;

	SortIndex(const std::tr1::array<int,256>& aa)
	:d(aa)
	{}
...
vector<PackType> huff_compress(const Image<byte>& im, const std::tr1::array<int,256>& h)
{
	...
	std::tr1::array<std::tr1::array<std::tr1::array<PackType, 20>, PackBits>, 256> fast_symbols;
	std::tr1::array<std::tr1::array<int, PackBits>, 256> fast_symbols_num_chunks;
...
template<class P> void huff_decompress(const vector<P>& b, const std::tr1::array<int,256>& h, Image<byte>& ret)
{
	vector<Huff*> terminals;
	Huff* table = create_tree(h, terminals);
...
class ReadPimpl
{
	...
	private:
		...
		std::tr1::array<int, 256> read_hist(std::istream& is);
...
:i(in)
{
	...
	row = 0;
	read_header(in);
	std::tr1::array<int,256> h = read_hist(in);
...
std::tr1::array<int, 256> ReadPimpl::read_hist(std::istream& is)
{
	std::tr1::array<int, 256> h;
	for (unsigned int i = 0; i < h.size(); i++) {
...
class WritePimpl
{
	...
	private:
		...
		void write_hist(std::ostream& os, const std::tr1::array<int, 256>& h);
...
void WritePimpl::write_hist(std::ostream& os, const std::tr1::array<int, 256>& h)
{
